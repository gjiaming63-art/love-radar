...
import { redeemScreenshotCode } from "@/lib/unlock-codes";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await redeemScreenshotCode(code, getClientKey(request), user?.id);
    return NextResponse.json(result, { status: result.success ? 200 : 400 });
  } catch (error) {
    console.error("redeem screenshot route failed:", error);
