  ...
  return (
    <main className="relative min-h-svh overflow-hidden px-4 py-5">
      ...
      <section className="relative mx-auto w-full max-w-5xl">
        <Link href="/" prefetch={false} className="inline-flex items-center gap-2 text-sm text-muted-foreground transition hover:text-foreground">
          ...
          <div className="space-y-5">
            <div className="rounded-lg border border-primary/30 bg-primary/10 p-5">
              <div className="flex items-center gap-2">
                ...
                <Badge className="border-primary/25 bg-primary/10 text-primary">恋爱占星师</Badge>
              </div>
              <h1 className="mt-5 text-4xl font-semibold tracking-normal sm:text-5xl">看看你们的关系星图</h1>
              <p className="mt-4 text-base leading-7 text-muted-foreground">
                输入两个人的出生信息，系统先计算核心星体与合盘相位，再生成一份恋爱关系洞察报告。
              </p>
            </div>
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                西方占星是象征性的自我观察工具，本功能仅供娱乐、反思和关系探索，不构成心理、法律或情感决策建议。
              </p>
            </div>
            <MiniAstrologyVisual />
          </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-accent/20 bg-card/70 p-5">
      ...
      <div className="relative mt-2 text-center">
        <p className="text-sm font-semibold">关系星图正在对齐</p>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">用星体相位观察吸引、沟通、拉扯和稳定感。</p>
      </div>
    </div>
  );
}
